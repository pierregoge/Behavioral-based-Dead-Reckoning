...
for i=1:length(lim_reg(:,1))
    ...
    m_speed(i,1) = (abs(diff_depth(i,1))./tan(m_pitch(i,1)))/((lim_reg(i,2)-lim_reg(i,1))/10);
    m_speed_ref_7(i,1) = mean(speed_ref(lim_reg(i,1):lim_reg(i,2),1));
    speed_up(lim_reg(i,1):lim_reg(i,2),1) = m_speed(i,1);
end
...
rmse_m_s = sqrt(sum((m_speed-m_speed_ref_7).^2)/length(m_speed(:,1)))

len_s= 0;
for i = 1:length(speed_up)

       if ~isnan(speed_up(i,1))
       len_s = len_s+1;
       buf_diff_pow(len_s,1)=(speed_up(i,1)-speed_ref(i,1)).^2;
       %buf_diff_pow(len_s,2)=(s(i,1)-speed_ref(i,1)).^2;
       end
end

rmse_s = sqrt(sum((buf_diff_pow))/len_s)
%rmse_s_2 = sqrt(sum((buf_diff_pow(:,2)))/len_s)
...
cc = 0;
for i=1:length(pitch_buf(:,1))
    if ~isnan(pitch_buf(i,1))
        cc = cc+1;
    end
    if ~isnan(pitch_buf_up(i,1))
        cc = cc+1;
    end
end
