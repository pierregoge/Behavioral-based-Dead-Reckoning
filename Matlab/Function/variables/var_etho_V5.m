function dives = var_etho_V5(dives,param)
...
%                 flag_s(:,:) = 0;
%                 flag_e(:,:) = 0;
            if flag_s(2,1) >= t(2,1)*X.P.sampling_rate && abs(X.P.data(flag_s(2,2),1)-X.P.data(flag_s(2,2)-flag_s(2,1),1)) < 0.3 %REST : compare flag number and sampling (in nb of sample)
...
%         end
        ...
        if c_behavior == 1 %If the time of end flag for up phase is superior to the threshold
            ...
            elseif nb_jerk >= nb_jerk_limit && X.P.data(j,1) > depth_ground_limit 
                %if abs(X.P.data(t_jerk_start_s,1)-X.P.data(j,1)) < 0.6 %Ground: compare flag number and sampling (in nb of sample)
