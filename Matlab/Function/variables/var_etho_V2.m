 function dives = var_etho_V2(dives,param)
...
%         end
        ...
        %SWIM phase -> DOWN, UP, REST, GROUND
        if c_behavior == 1 %If the time of end flag for up phase is superior to the threshold
            ...
            elseif nb_jerk >= nb_jerk_limit && X.P.data(j,1) > depth_ground_limit 
                if abs(X.P.data(t_jerk_start_s,1)-X.P.data(j,1)) < 0.4 %DOWN: compare flag number and sampling (in nb of sample)
