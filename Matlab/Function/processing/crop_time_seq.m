function [seq_out] = crop_time_seq(seq,lim_inf,lim_sup)

%     seq_out = crop_seq(seq,lim_inf,lim_sup)
%     Crop a sequence sctructure or variable sctructure between two time limits.
%
%		Inputs:
...
if isstruct(seq) && isfield(seq,'data')  %Check if the input is a data structure
    ...
    y =data(start_crop:end_crop,:);    % Simple downsampling function
...
%     end
    ...
    for i=1:length(Z)   % For loop the compute each variable field
        ...
        if isstruct([X.(Z{i})]) && ~strcmp(([X.(Z{i}).name]),'B')
            ...
            crop_len = length(lim_inf:lim_sup)-1;
            start_crop = (seq.tstart+lim_inf)*fs;
            end_crop =  (seq.tstart+lim_inf+crop_len)*fs-1;
        ...
        elseif isstruct([X.(Z{i})]) && strcmp(([X.(Z{i}).name]),'B')   % If Behavior data are available crop is different
         ...
         for j=1:length(data_etho(:,1))
             ...
             if (seq.tstart+lim_inf)*fs > data_etho(j,3) 
                 lim_inf_e = j+1;
                 start_b = (seq.tstart+lim_inf)*fs-data_etho(j,3);
             end
             if (seq.tstart+lim_inf+crop_len)*fs-1 > data_etho(j,2) 
                 lim_sup_e = j;
                 end_b = (seq.tstart+lim_inf+crop_len)*fs-1 - data_etho(j,3);
             end               
         end
